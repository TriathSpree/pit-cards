...
async function sharedGetLeaderboard() {
  try {
    ...
    return (d.records||[]).map(rec => ({
      ...
      dailyKey: rec.fields.dailyKey||"", dailyPts: rec.fields.dailyPts||0,
      stats_solo: JSON.parse(rec.fields.stats_solo||"{}"),
      stats_4j: JSON.parse(rec.fields.stats_4j||"{}"),
    }));
  } catch { return []; }
}
...
function applyPlay(state,who,cardId,action,plbl){const s=JSON.parse(JSON.stringify(state));const actor=s[who],target=who==="player"?s.ai:s.player;const idx=actor.hand.indexOf(cardId);if(idx!==-1)actor.hand.splice(idx,1);const L=who==="player"?plbl:"Victor";if(action==="borne"){actor.bornes=(actor.bornes||[]).concat(cardId);actor.km+=getCard(cardId).km;s.log.unshift({text:cEmoji(cardId)+" "+L+" avance de "+getCard(cardId).km+" km (total: "+actor.km+" km)",who});}else if(action==="parade"){const c=getCard(cardId);if(c.id==="fin_limite"){actor.limitee=false;actor.lastLimite=cardId;s.log.unshift({text:cEmoji(cardId)+" "+L+" retire la limitation.",who});}else if(c.id==="feu_vert"){if(actor.attaque==="feu_rouge")actor.attaque=null;actor.started=true;actor.lastCard=cardId;s.log.unshift({text:cEmoji("feu_vert")+" "+L+" passe au feu vert !",who});}else{actor.attaque=null;actor.lastCard=cardId;s.log.unshift({text:cEmoji(cardId)+" "+L+" répare : "+c.label,who});}}else if(action==="attaque"){const c=getCard(cardId);if(c.id==="limite"){target.limitee=true;target.lastLimite=cardId;}else{target.attaque=cardId;target.lastCard=cardId;target.wasAttacked=true;}s.log.unshift({text:cEmoji(cardId)+" "+L+" joue : "+c.label+" sur l'adversaire !",who});}else if(action==="botte"){const bo=getCard(cardId);actor.bottes.push(cardId);const co=Array.isArray(bo.counters)?bo.counters:[bo.counters];if(actor.attaque&&co.includes(actor.attaque)){actor.attaque=null;actor.lastCard=actor.started?"feu_vert":null;}if(bo.id==="prioritaire"){actor.limitee=false;actor.lastLimite="fin_limite";}s.log.unshift({text:cEmoji(cardId)+" "+L+" joue la botte : "+bo.label+" !",who});}return s;}
...
function discardCard(s,who,id,plbl){s=JSON.parse(JSON.stringify(s));const i=s[who].hand.indexOf(id);if(i!==-1)s[who].hand.splice(i,1);s.discard.push(id);s.log.unshift({text:cEmoji(id)+" "+(who==="player"?plbl:"Victor")+" défausse : "+getCard(id)?.label+".",who});return s;}
...
function LeaderboardPanel({dark,th,medals,leaderboard,loadingLB,initialMode}){
  ...
  const getScores=(mode)=>{
    ...
    return leaderboard
      .map(r=>({
        nom:r.nom,
        pts:mode==="solo"?(r.stats_solo?.wins||0)*100+(r.stats_solo?.played||0)*10:
            (r.stats_4j?.wins||0)*100+(r.stats_4j?.played||0)*10,
        wins:mode==="solo"?(r.stats_solo?.wins||0):(r.stats_4j?.wins||0),
        played:mode==="solo"?(r.stats_solo?.played||0):(r.stats_4j?.played||0),
        objPts:r.objPts||0,
      }))
      .filter(r=>r.played>0)
  ...
  const scores=getScores(lbMode);

  return(
    <div>
      <div style={{display:"flex",gap:"4px",marginBottom:"12px"}}>
        {["solo","4j","online"].map(m=>(
          <button key={m} onClick={()=>setLbMode(m)} style={{flex:1,padding:"8px",border:"2px solid "+(lbMode===m?th.title:th.border),borderRadius:"10px",background:lbMode===m?(dark?"rgba(224,112,112,0.15)":"rgba(139,0,0,0.08)"):"transparent",color:lbMode===m?th.title:th.subtext,fontFamily:"Georgia,serif",fontSize:"12px",fontWeight:"bold",cursor:"pointer"}}>
      ...
      {lbMode==="online"?(
        ...
        <>
          ...
          <div style={{background:th.cardBg,border:"2px solid "+th.border,borderRadius:"12px",overflow:"hidden"}}>
            <div style={{display:"grid",gridTemplateColumns:"36px 1fr 60px 50px",padding:"8px 10px",background:dark?"rgba(0,0,0,0.3)":"rgba(139,0,0,0.08)",fontSize:"9px",fontWeight:"bold",textTransform:"uppercase",letterSpacing:"1px",color:th.subtext}}>
              <span>#</span><span>Joueur</span><span style={{textAlign:"right"}}>Victoires</span><span style={{textAlign:"right"}}>Parties</span>
            </div>
            {loadingLB&&<div style={{padding:"20px",textAlign:"center",fontSize:"11px",color:th.subtext}}>⏳ Chargement...</div>}
            {!loadingLB&&scores.length===0&&<div style={{padding:"20px",textAlign:"center",fontSize:"11px",color:th.subtext,fontStyle:"italic"}}>Aucun joueur dans ce mode encore !</div>}
            {!loadingLB&&scores.map((s,i)=>(
              <div key={i} style={{display:"grid",gridTemplateColumns:"36px 1fr 60px 50px",padding:"10px",borderTop:"1px solid "+th.border,alignItems:"center",background:i===0?(dark?"rgba(212,172,13,0.08)":"rgba(212,172,13,0.05)"):"transparent"}}>
                ...
                <span style={{fontSize:"12px",fontWeight:"bold",color:i===0?th.gold:th.accent,textAlign:"right"}}>{s.wins}</span>
                <span style={{fontSize:"11px",color:th.subtext,textAlign:"right"}}>{s.played}</span>
              </div>
            ))}
          </div>
        </>
      ...
      {isMobile&&(
        <div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px",marginBottom:"6px"}}>
            <div style={!isAiTurn?th.playerBd:th.aiBd}><div style={{fontWeight:"bold",fontSize:"10px",marginBottom:"3px",textAlign:"center",color:th.text}}>{progress?.avatar||"🧑"} {dn.toUpperCase()}</div>{renderStatus(state.player)}</div>
      ...
      {!isMobile&&(
        <div>
          <div style={{display:"grid",gridTemplateColumns:"1fr auto auto 1fr",gap:"10px",marginBottom:"10px",alignItems:"stretch"}}>
            <div style={!isAiTurn?th.playerBd:th.aiBd}><div style={{fontWeight:"bold",fontSize:"13px",marginBottom:"6px",textAlign:"center",letterSpacing:"2px",color:th.text}}>{progress?.avatar||"🧑"} {dn.toUpperCase()}</div>{renderStatus(state.player)}</div>
